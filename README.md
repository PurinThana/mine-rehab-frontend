...
**ลิงก์เอกสารไม่ได้ชี้ไป Cloudinary ตรงๆ** แต่ใช้ `documentsApi.fileUrl(id)` ซึ่งชี้ไป
`GET /documents/:id/file` เพราะบัญชี Cloudinary แบบฟรีบล็อกการส่ง PDF ออกทาง URL
สาธารณะ (เปิดลิงก์ตรงจะได้ 401) ฝั่ง backend ดึงไฟล์ด้วย signed URL แล้วส่งต่อ
พร้อม Content-Type และชื่อไฟล์ที่ถูกต้อง — ดูรายละเอียดใน README ของ backend

## รูปพื้นหลังส่วนหัวเว็บ (Hero)

ตั้งได้จากหน้าแอดมิน → ข้อมูลโครงการ → “รูปพื้นหลังส่วนหัวเว็บ (Hero)”
เก็บใน `sites.hero_image_url`

`Hero.jsx` วางรูปเป็นชั้น `absolute inset-0` แบบ `object-cover` แล้ว**ทับด้วย
gradient สีเข้มอีกชั้น** — จำเป็นเพราะถ้าวางรูปเปล่าๆ ตัวหนังสือสีครีมจะจมหาย
ไปกับรูปที่สว่าง ชั้นทับทำให้อ่านออกไม่ว่าจะใช้รูปอะไร

ไม่ตั้งรูปก็ได้ — จะเหลือพื้นสีเขียวเข้ม `bg-forest-800` เหมือนดีไซน์เดิม
